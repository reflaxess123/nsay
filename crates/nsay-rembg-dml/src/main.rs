...
//   4. Run inference (DirectML EP — any DX12 GPU on Windows 10+).
...
fn main() {
    if let Err(e) = run() {
        let _ = writeln!(std::io::stderr(), "nsay-rembg-dml error: {e:#}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<()> {
    ...
    // DirectML EP. Hardware-agnostic Windows GPU (NVIDIA / AMD / Intel) over DX12.
    let mut session = Session::builder()
        .map_err(|e| anyhow::anyhow!("ort session builder: {e}"))?
        .with_execution_providers([DirectMLExecutionProvider::default().build().error_on_failure()])
        .map_err(|e| anyhow::anyhow!("with_execution_providers (dml): {e}"))?
