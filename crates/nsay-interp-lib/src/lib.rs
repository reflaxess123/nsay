// Shared frame interpolation sidecar pipeline (RIFE 4.9). Backend bins
// (nsay-interp-cpu / -cuda / -dml) call `run(setup)` with a closure that
// registers their execution provider on the SessionBuilder.
//
...
/// Run the streaming interp pipeline. `provider_setup` registers the EP —
/// CPU passes `Ok`, GPU bins build a CUDA / DirectML provider.
    ...
    F: FnOnce(SessionBuilder) -> ort::Result<SessionBuilder>,
{
    ...
    // RIFE exports vary across uploaders. yuvraj108c/rife-onnx ensemble:
    // img0, img1, timestep.
    ...
    loop {
        ...
        for k in 1..args.factor {
            ...
            // scalar, NOT a 4D broadcast of [1,1,H,W]. Mismatch produces
            // a cryptic "Invalid rank for input: timestep" at session.run.
...
fn crop_chw_to_rgb(chw: &[f32], pad_w: u32, _pad_h: u32, src_w: u32, src_h: u32) -> Vec<u8> {
    let mut out = vec![0u8; (src_w * src_h * 3) as usize];
    let plane = (pad_w * _pad_h) as usize;
    for y in 0..src_h {
        for x in 0..src_w {
